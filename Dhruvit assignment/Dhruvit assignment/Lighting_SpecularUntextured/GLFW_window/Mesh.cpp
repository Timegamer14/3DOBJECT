...
void Mesh::LoadMeshFromFile(std::string filename)
{
	...
	if (myfile.is_open())
	{
		...
		{
			for (int vertComponent = 0; vertComponent < 6; ++vertComponent)
			{
				myfile >> tempfloat;
...
// CONSTRUCTOR
Mesh::Mesh(GLSLProgram* shaders, std::string fileName)
{
...
void Mesh::Draw()
{
	...
	//material shinyness
	shaders->setUniform("shinyness", shiny);
	//material specular "power" : increasing this focuses the specular reflection size
	shaders->setUniform("specularPower", specularPower);
